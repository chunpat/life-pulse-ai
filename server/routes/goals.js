...
router.post('/', authenticateToken, async (req, res) => {
  try {
    const { goalType, title, rewardTitle, officialPlanTemplateId } = req.body;
    const goal = await createGoalForUser(req.user.id, { goalType, title, rewardTitle, officialPlanTemplateId });
...
router.post('/:id/set-primary', authenticateToken, async (req, res) => {
  try {
    const goal = await setPrimaryGoal(req.user.id, req.params.id);
    res.json(goal);
  } catch (error) {
    const statusCode = getGoalErrorStatus(error.message);
    res.status(statusCode).json({ message: error.message, error: error.message });
